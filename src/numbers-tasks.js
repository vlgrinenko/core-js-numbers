...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(scal / (module1 * module2));
}
...
function isPrime(n) {
  ...
  for (let index = 2; index < Math.round(n / 2); index += 1) {
    if (n % index === 0) {
      return false;
    }
  }
...
function toExponential(number, fractionDigits) {
  return String(number.toExponential(fractionDigits));
}
